...
int dns_confirm(unsigned char *readbuf, size_t len)
{
    if(len < 12)
        return 0;
    if(((readbuf[2]&0xf8) == 0x80) && ((readbuf[3]&0x0f)==0x00))
	return 1;
    else
        return 0;
}
...
int csv4rdns_process(fieldset_t *fs)
{
    ...
    // if the response is noerr dns response, print the saddr 
    field_t *f = &(fs->fields[data_index]);
    //unsigned char *buf = (unsigned char *)f->value.ptr;
    if(dns_confirm((unsigned char *)f->value.ptr, f->len)){
		...
		//fprintf(file, " %u", *(unsigned short *)buf);
		//fprintf(file, " %u", ((buf[3]&0x80) == 0x80));
		//fprintf(file, " %u", ((buf[3]&0x20) == 0x20));
		//fprintf(file, " %u\n", (buf[3]&0x0f));
    	fprintf(file, "\n");
    }
	fflush(file);
